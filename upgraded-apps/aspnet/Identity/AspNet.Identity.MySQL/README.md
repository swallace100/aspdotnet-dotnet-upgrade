# AspNet Identity MySQL App Conversion

## Introduction

This app is a MySQL library that is compatible with .NET 10.

It was created using the following PowerShell command:

```PowerShell
dotnet new classlib -n AspNet.Identity.MySQL -f net10.0
```

## Dev notes

MySQLIdentity.sql was brought over in its original form.

Instead of having definition files for each table, the definition is now handled by a ModelBuilder.

The MySQLDatabbase.cs file is also replaced with the IServiceCollection-based function, AddMySqlIdentity, in ServiceCollectionExtension.cs.
